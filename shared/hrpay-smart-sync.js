...
(function(g){
  ...
  function smartPush(opts,records,remote,onStatus,migrated){var tool=opts.tool,url=text(opts.url).trim();return Promise.all([buildBuckets(records),hash(stable(opts.meta||{}))]).then(function(parts){
    var local=parts[0],metaHash=parts[1],last=readState(opts.stateKey||tool)||{},lastH=last.hashes||{},remoteH=remote.hashes||{},remoteMetaHash=remote.metaHash||'',changed=[],deleted=[],remoteChanged=[],conflicts=[],ctx=opts.context||g.__HRPAY_SYNC_CONTEXT||{},allowDelete=/delete/i.test(String((opts.context||g.__HRPAY_SYNC_CONTEXT||{}).reason||''));if(migrated){lastH={};remoteH={};remoteMetaHash='';}
    var keys={};Object.keys(local).concat(Object.keys(remoteH)).forEach(function(k){keys[k]=1;});Object.keys(keys).forEach(function(k){var lh=local[k]&&local[k].hash||'',rh=remoteH[k]||'',bh=lastH[k]||'';if(lh&&rh&&lh===rh)return;if(!bh){if(lh&&!rh)changed.push(k);else if(!lh&&rh)remoteChanged.push(k);else if(lh&&rh&&lh!==rh)conflicts.push(k);return;}var lc=lh!==bh,rc=rh!==bh;if(lc&&!rc){if(lh)changed.push(k);else if(rh){if(allowDelete)deleted.push(k);else remoteChanged.push(k);}}else if(!lc&&rc)remoteChanged.push(k);else if(lc&&rc&&lh!==rh)conflicts.push(k);});
    ...
    return chain.then(function(){var hashes={},counts={};Object.keys(local).forEach(function(k){hashes[k]=local[k].hash;counts[k]=local[k].count;});Object.keys(remoteH).forEach(function(k){if(!hashes[k]&&deleted.indexOf(k)<0){hashes[k]=remoteH[k];counts[k]=Number((remote.counts||{})[k])||0;}});var rc=Object.keys(counts).reduce(function(n,k){return n+(Number(counts[k])||0);},0),meta=Object.assign({},opts.meta||{},{_smartMetaHash:metaHash});var metaChanged=metaHash!==remoteMetaHash||migrated||changed.length||deleted.length;if(!metaChanged&&!changed.length&&!deleted.length){writeState(opts.stateKey||tool,{hashes:remoteH,counts:remote.counts||{},metaHash:remoteMetaHash,updatedAt:now()});status(onStatus,'雲端已是最新，不需重傳','ok');return{ok:true,skipped:true,unchanged:records.length};}
      return post(url,{action:'smartCommit',tool:tool,uploadId:uploadId,baseHashes:remoteH,baseMetaHash:remoteMetaHash,hashes:hashes,counts:counts,recordCount:rc,meta:meta,summary:opts.summary||{},context:opts.context||g.__HRPAY_SYNC_CONTEXT||{}}).then(function(d){var out={ok:true,recordCount:rc,uploaded:sent,unchanged:Math.max(0,records.length-sent),changedBuckets:changed.length,deletedBuckets:deleted.length,migrated:!!migrated,timestamp:(d&&(d.timestamp||d.updatedAt))||now()};writeState(opts.stateKey||tool,{hashes:hashes,counts:counts,metaHash:metaHash,updatedAt:out.timestamp});status(onStatus,'完成｜上傳 '+sent+(deleted.length?'｜刪除 '+deleted.length+' bucket':'')+'｜保留其他雲端歷史','ok');return out;});});
  });}
  function pull(opts){opts=opts||{};var url=text(opts.url).trim(),tool=text(opts.tool).trim(),localRows=sortRows(opts.localRecords||[]),onStatus=opts.onStatus;if(!url||!tool)return Promise.reject(new Error('GAS URL/tool missing'));status(onStatus,'智慧下載：比對雲端差異…','busy');return manifest(url,tool).then(function(remote){remote=remote||{};if(!remote.exists&&remote.legacy){return legacyPull(url,tool,opts,onStatus).then(function(old){var merged=merge(opts,localRows,old.records||[]);return Promise.resolve(opts.apply?opts.apply(merged,old.meta||{}):null).then(function(){var o=Object.assign({},opts,{records:merged});return smartPush(o,merged,remote,onStatus,true).then(function(m){return{ok:true,records:merged,downloaded:(old.records||[]).length,migrated:true,pushResult:m};});});});}if(!remote.exists){status(onStatus,'雲端尚無資料','warn');return{ok:false,noCloud:true,records:localRows};}
      return buildBuckets(localRows).then(function(local){var last=readState(opts.stateKey||tool)||{},lastH=last.hashes||{},remoteH=remote.hashes||{},out={},downloaded=0,unchanged=0,pending=0,keys={};Object.keys(remoteH).concat(Object.keys(local)).forEach(function(k){keys[k]=1;});var list=Object.keys(keys).sort(),chain=Promise.resolve();list.forEach(function(k,idx){chain=chain.then(function(){var lb=local[k],rh=remoteH[k]||'';if(lb&&rh&&lb.hash===rh){out[k]=lb.records;unchanged+=lb.count;return;}if(lb&&!rh){if(lastH[k]&&lb.hash===lastH[k])return;out[k]=lb.records;pending+=lb.count;return;}if(!rh)return;status(onStatus,'下載變更 '+(idx+1)+'/'+list.length+' · '+k,'busy');return jsonFetch(url+'?action=smartBucket&tool='+enc(tool)+'&bucket='+enc(k)).then(function(j){var bd=dataOf(j)||{},rows=bd.records||[];if(bd.hash&&bd.hash!==rh)throw new Error('Cloud changed during download; retry.');downloaded+=rows.length;if(lb&&lb.hash!==rh&&lb.hash!==lastH[k]){if(!opts.mergeRecords){var remoteBy={};rows.forEach(function(r){remoteBy[semanticKey(r)]=r;});lb.records.forEach(function(r){var other=remoteBy[semanticKey(r)];if(other&&stable(r)!==stable(other)&&stamp(r)===stamp(other))throw new Error('Concurrent edits to '+semanticKey(r)+'. Review both copies before syncing.');});}out[k]=merge(opts,lb.records,rows);}else out[k]=rows;});});});return chain.then(function(){var merged=sortRows(Object.keys(out).reduce(function(a,k){return a.concat(out[k]||[]);},[]));var contentChanged=stable(merged)!==stable(localRows);return Promise.resolve(contentChanged&&opts.apply?opts.apply(merged,remote.meta||{}):null).then(function(){writeState(opts.stateKey||tool,{hashes:remoteH,counts:remote.counts||{},metaHash:remote.metaHash||'',updatedAt:now()});status(onStatus,'完成｜下載 '+downloaded+'｜未變 '+unchanged+(pending?'｜本機待上傳 '+pending:''),'ok');return{ok:true,records:merged,downloaded:downloaded,unchanged:unchanged,pendingUpload:pending,meta:remote.meta||{}};});});});
    });}
  var queues={};function queued(fn,opts){var key=(opts&&opts.stateKey)||(opts&&opts.tool)||'default';var job=(queues[key]||Promise.resolve()).catch(function(){}).then(function(){return fn(opts);});queues[key]=job;return job;}g.HRPaySmartSync={version:VERSION,push:function(o){return queued(push,o);},pull:function(o){return queued(pull,o);},buildBuckets:buildBuckets,mergeRows:mergeRows,semanticKey:semanticKey,bucketKey:bucketKey,stable:stable};
})(window);
