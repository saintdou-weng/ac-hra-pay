...
(function(g){
  ...
  var VERSION='1.2', STATE_PREFIX='ac_hrpay_smart_sync_v1_', nativeFetch=g.fetch?g.fetch.bind(g):null;
  ...
  function smartPush(opts,records,remote,onStatus,migrated){var tool=opts.tool,url=text(opts.url).trim();return Promise.all([buildBuckets(records),hash(stable(opts.meta||{}))]).then(function(parts){
    var local=parts[0],metaHash=parts[1],last=readState(tool)||{},lastH=last.hashes||{},remoteH=remote.hashes||{},remoteMetaHash=remote.metaHash||'',changed=[],deleted=[],remoteChanged=[],conflicts=[],ctx=opts.context||g.__HRPAY_SYNC_CONTEXT||{},allowDelete=/delete/i.test(String((opts.context||g.__HRPAY_SYNC_CONTEXT||{}).reason||''));if(migrated){lastH={};remoteH={};remoteMetaHash='';}
